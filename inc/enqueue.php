...
function wp10ms_enqueue_files() {
    ...
    if ( is_singular( 'service' ) ) { wp_enqueue_style( 'service-single', get_template_directory_uri() . '/assets/css/service-single.css', array(), _S_VERSION ); }
    // add author page specific styles
    if ( is_author() ) { wp_enqueue_style( 'author-style', get_template_directory_uri() . '/assets/css/author-page.css', array(), _S_VERSION ); }
    // add search page specific styles
    if ( is_search() ) { wp_enqueue_style( 'wp10ms-search-page', get_template_directory_uri() . '/assets/css/search.css', array(), _S_VERSION ); }
    // minimal css for all pages
    wp_enqueue_style( 'minimal', get_template_directory_uri() . '/assets/css/minimal.css', array(), _S_VERSION );
...
add_action( 'wp_enqueue_scripts', 'wp10ms_enqueue_files' );